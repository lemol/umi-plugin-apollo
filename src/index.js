...
const joinApolloPath = api => path =>
  join(api.paths.tmpDirPath, "apollo", path);
const joinAbsApolloPath = api => path =>
  join(api.paths.absTmpDirPath, "apollo", path);
const joinApolloTemplatePath = __ => path =>
  join(__dirname, "../template/umi/apollo", path);
...
export default function(api, opts = {}) {
  ...
  api.addVersionInfo(
    dependencies.map(
      pkgName => `${pkgName}@${require(`${pkgName}/package`).version}`
    )
  );
